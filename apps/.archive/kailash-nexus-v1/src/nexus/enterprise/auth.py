...
Built on Kailash SDK authentication nodes and integrates with kailash-mcp auth system.
"""
...
# Import core SDK authentication components
...
class EnterpriseAuthManager:
    ...
    Integrates with proven kailash-mcp authentication system when available.
    """
    ...
    def __init__(
        ...
        # Initialize SDK-based authentication workflows
        self._auth_workflows = {}
        self._setup_sdk_auth_workflows()
    ...
    def _setup_sdk_auth_workflows(self):
        """Set up SDK-based authentication workflows for each provider."""
        for provider_config in self.providers:
            # Handle both dict and AuthProvider object formats
            if hasattr(provider_config, "type"):
                provider_type = provider_config.type
                provider_name = getattr(provider_config, "name", provider_type)
                config_dict = (
                    provider_config.config if hasattr(provider_config, "config") else {}
                )
            else:
                provider_type = provider_config.get("type", "simple")
                provider_name = provider_config.get("name", provider_type)
                config_dict = provider_config

            if provider_type == "simple" or provider_type == "local":
                # Create local authentication workflow
                ...
                workflow.add_node(
                    "AuditLogNode", "audit", {"event_type": "local_authentication"}
                )
                workflow.add_connection("authenticate", "result", "audit", "event")
                self._auth_workflows[provider_name] = workflow.build()
                logger.info(f"Created local auth workflow: {provider_name}")

            elif provider_type == "oauth2":
                # Create OAuth2 authentication workflow
                workflow = WorkflowBuilder()
                workflow.add_node(
                    "OAuth2Node",
                    "oauth_auth",
                    {
                        ...
                        "userinfo_url": config_dict.get("userinfo_url"),
                    },
                )
                workflow.add_node(
                ...
                workflow.add_connection("sync_user", "result", "audit", "event")
                self._auth_workflows[provider_name] = workflow.build()
                logger.info(f"Created OAuth2 auth workflow: {provider_name}")

            elif provider_type == "ldap":
                # Create LDAP authentication workflow
                workflow = WorkflowBuilder()
                workflow.add_node(
                    ...
                    "ldap_auth",
                    {
                        "directory_type": "ldap",
                        "connection_config": {
                            "server_url": f"ldap://{config_dict.get('host', 'localhost')}:{config_dict.get('port', 389)}",
                            "base_dn": config_dict.get("base_dn", "dc=example,dc=com"),
                            "bind_dn": config_dict.get("bind_dn"),
                            "bind_password": config_dict.get("bind_password"),
                        },
                        "filter_config": {
                            "user_filter": config_dict.get(
                ...
                workflow.add_connection("ldap_auth", "user", "sync_user", "user_data")
                workflow.add_connection("sync_user", "result", "audit", "event")
                self._auth_workflows[provider_name] = workflow.build()
                logger.info(f"Created LDAP auth workflow: {provider_name}")

            elif provider_type == "enterprise":
                # Create enterprise authentication workflow using EnterpriseAuthProviderNode
                workflow = WorkflowBuilder()
                workflow.add_node(
                    "EnterpriseAuthProviderNode",
                    "enterprise_auth",
                    {
                        "providers": config_dict.get(
                            "auth_providers", ["sso", "ldap", "mfa"]
                        ),
                ...
                workflow.add_node(
                    "AuditLogNode", "audit", {"event_type": "enterprise_authentication"}
                )
                workflow.add_connection(
                    "enterprise_auth", "result", "threat_check", "auth_data"
                )
                workflow.add_connection("threat_check", "result", "audit", "event")
                self._auth_workflows[provider_name] = workflow.build()
                logger.info(f"Created enterprise auth workflow: {provider_name}")
    ...
    def _init_auth_workflows(self):
        ...
        self.ldap_workflow.add_node(
            "DirectoryIntegrationNode", "ldap_auth", {"directory_type": "ldap"}
        )
        self.ldap_workflow.add_node(
            "UserManagementNode", "sync_user", {"operation": "create_or_update"}
        )
    ...
    async def authenticate(
        ...
        """Authenticate user with credentials using SDK workflows.
        ...
        # Use SDK workflow-based authentication
        return await self._authenticate_with_sdk_workflows(credentials, provider)

    async def _authenticate_with_sdk_workflows(
        self, credentials: Dict[str, Any], provider: Optional[str] = None
    ) -> Optional[AuthToken]:
        """Authenticate using SDK-based authentication workflows."""
        try:
            # Get the appropriate authentication workflow
            auth_workflow = self._auth_workflows.get(provider)
            if not auth_workflow:
                logger.warning(
                    f"No authentication workflow found for provider: {provider}"
                )
                return await self._authenticate_basic(credentials, provider)
            ...
            workflow_params = {
                ...
                "sync_user": {"provider": provider},
                "audit": {"user_id": credentials.get("username", "unknown")},
                "ldap_auth": {"action": "authenticate", **credentials},
            }

            # Execute authentication workflow
            results, run_id = runtime.execute(auth_workflow, parameters=workflow_params)

            # Check if authentication was successful
            if results and results.get("audit", {}).get("success", False):
                user_info = results.get("sync_user", {})
                if user_info:
                    # Create token
                    ...
                    self._user_sessions[user_id].append(token.token)

                    logger.info(
                        f"User authenticated via SDK workflow: {user_id} via {provider}"
                    )
                    return token

            logger.warning(
                f"SDK workflow authentication failed for provider: {provider}"
            )
            return None

        except Exception as e:
            logger.error(f"SDK workflow authentication error: {e}")
    ...
    async def _authenticate_basic(
        ...
        mfa_required = (
            self.mfa_config.get("required")
            if isinstance(self.mfa_config, dict)
            else getattr(self.mfa_config, "required", False)
        )
        if mfa_required and not credentials.get("mfa_bypass"):
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        for provider in self.providers:
            ...
            provider_type = (
                provider.type
                if hasattr(provider, "type")
                else provider.get("type", "unknown")
            )
            provider_status[provider_type] = "healthy"

        mfa_enabled = (
            self.mfa_config.required
            if hasattr(self.mfa_config, "required")
            else (
                self.mfa_config.get("required", False)
                if isinstance(self.mfa_config, dict)
                else False
